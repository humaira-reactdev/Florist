...
const CartComponent = () => {
    ...
    onValue(starCountRef, (snapshot) => {
      ...
      snapshot.forEach((item) => {
        ...
        if (!arr.some(product => product.key === item.key)) {
          ...
          setQuantities((prev) => ({ ...prev, [item.key]: prev[item.key] || item.val().quantity || 1 }));
        }
      });
      setProducts(arr);
    });
  ...
  const handleIncrement = (id) => {
    setQuantities((prev) => {
      ...
      update(ref(db, `cartProduct/${id}`), { quantity: newQuantity });
  ...
  const handleClearCart = () => {
    products.forEach(item => remove(ref(db, 'cartProduct/' + item.key)));
    setProducts([]);
    setQuantities({});
  };
        ...
        <div className="w-full lg:w-2/3">
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex gap-4">
              <button className="text-pink-500" onClick={handleClearCart}>
                🗑 CLEAR SHOPPING CART
              </button>
              <button className="text-pink-500" onClick={() => setProducts([...products])}>⟳ UPDATE CART</button>
            </div>
          </div>
        </div>
